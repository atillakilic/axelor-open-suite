...
public class DashboardPurchaseController {
  ...
  public void productOverview(ActionRequest request, ActionResponse response)
    ...
    List<PurchaseRequestRu> PurchaseRequestRuLine =
        Beans.get(PurchaseRequestRuRepository.class)
            .all()
            .filter("self.requestDate >= ? AND self.requestDate <= ?", fromDate, toDate)
            .fetch();

    List<DashboardProductOverviewRu> ProductOverviewList =
        new ArrayList<DashboardProductOverviewRu>();
  ...
  public void supplierBalance(ActionRequest request, ActionResponse response)
      throws AxelorException {
    DashboardPurchaseRu dashboard = request.getContext().asType(DashboardPurchaseRu.class);

    List<RequestSupplierListRu> supplierLineList =
        Beans.get(RequestSupplierListRuRepository.class).all().fetch();

    List<DashboardSupplierBalanceRu> dashboardSupplierBalanceRu =
        new ArrayList<DashboardSupplierBalanceRu>();
  ...
  public void supplierCost(ActionRequest request, ActionResponse response) throws AxelorException {
    ...
    List<PurchaseRequestRu> purchaseRequestRuList =
        Beans.get(PurchaseRequestRuRepository.class)
            .all()
            .filter("self.requestDate >= ? AND self.requestDate <= ?", fromDate, toDate)
            .fetch();

    List<DashboardSupplierCostRu> dashboardSupplierCostRuList =
        new ArrayList<DashboardSupplierCostRu>();
    ...
    for (PurchaseRequestRu purchaseRequestRu : purchaseRequestRuList) {
      ...
      for (PurchaseRequestLineRu purchaseRequestLineRu :
        ...
        for (RequestSupplierListRu requestSupplierListRu :
            purchaseRequestLineRu.getSupplierLineList()) {
          ...
          for (DashboardSupplierCostRu dashboardSupplierCostRu : dashboardSupplierCostRuList) {
            ...
            if (supplier.equals(supplierAlredyfound)) {
              ...
              if (PurchasePaymentRu == null) {
                dashboardSupplierCostRu.setNotPaid(
                    dashboardSupplierCostRu.getNotPaid().add(requestSupplierListRu.getTotalCost()));
                dashboardSupplierCostRu.setTotalCost(
                    dashboardSupplierCostRu
                        .getTotalCost()
                        .add(requestSupplierListRu.getTotalCost()));
              } else {
                if (PurchasePaymentRu.getPaymentMethod() == 1) {
                  dashboardSupplierCostRu.setPaidBank(
                      dashboardSupplierCostRu
                          .getPaidBank()
                          .add(requestSupplierListRu.getTotalCost()));
                  dashboardSupplierCostRu.setTotalCost(
                ...
                if (PurchasePaymentRu.getPaymentMethod() == 2) {
                  dashboardSupplierCostRu.setPaidCash(
                      dashboardSupplierCostRu
                          .getPaidCash()
                          .add(requestSupplierListRu.getTotalCost()));
                  dashboardSupplierCostRu.setTotalCost(
